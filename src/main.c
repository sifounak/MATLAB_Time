...
static Layer *s_batt_layer, *s_static_layer, *s_weather_layer, *s_weather_layer_unanimated, *s_logo;
...
static void inbox_received_handler(DictionaryIterator *iter, void *context) {
  ...
  if (use_celsius_t) {
    ...
    if (temperature_in_c_t) {
      ...
      snprintf(temp_buffer, sizeof(temp_c_buffer), "%d C", (int)temperature_in_c_t->value->int32);
    }
...
static void main_window_load(Window *window) {
  ...
  */
  ...
  //layer_set_update_proc(bitmap_layer_get_layer(s_logo_bitmap_layer), logo_update_proc);
  load_first_image();
...
static void main_window_unload(Window *window) {
  ...
  //gbitmap_destroy(s_mask);
  //gbitmap_sequence_destroy(s_sequence_mask);
